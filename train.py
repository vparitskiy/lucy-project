...
class DataTrainingArguments:
    ...
    dataset_path: Optional[str] = field(
        default=None,
        metadata={
            "help": "The name of the dataset on local disk to use (via the datasets library)."
        },
    )
...
def main():
    ...
    num_training_steps = steps_per_epochs * training_args.num_train_epochs
    warmup_steps = int(num_training_steps * 0.1)
    scheduler = transformers.get_cosine_schedule_with_warmup(
        optimizer, num_warmup_steps=warmup_steps, num_training_steps=num_training_steps
    )
    print(steps_per_epochs, num_training_steps, warmup_steps)
    ...
    if training_args.do_train:
        ...
        trainer.save_model(
            os.path.join(training_args.output_dir, "result")
        )  # Saves the tokenizer too for easy upload  # Saves the tokenizer too for easy upload
